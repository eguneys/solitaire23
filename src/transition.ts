...
import transition_vertex_shader from './transition.vert'
import transition_fragment_shader from './transition.frag'

let transition_shader: Shader
...
const texture2_uniform = 'u_texture2'
const mask_texture_uniform = 'u_mask_texture'
...
export class Transition {
  ...
  render_with_m(target: Target, matrix: Mat4x4) {
    ...
    if (!transition_shader) {
      transition_shader = Shader.create([transition_vertex_shader, 
    ...
    if (!this.material) {
      this.material = Material.create(transition_shader)
    }
    ...
    pass.material.set_texture(texture2_uniform, this.texture2)
    pass.material.set_texture(mask_texture_uniform, this.mask_texture)
    pass.material.set_sampler_at_location(0, this.sampler)
    pass.material.set_sampler_at_location(1, this.sampler)
    pass.material.set_sampler_at_location(2, this.sampler)
